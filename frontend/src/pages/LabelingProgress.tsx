...
export default function LabelingProgress() {
  ...
  const isSkipped = isQualityCheck && skipLabeling
    ...
    <div>
      ...
      <div className="page-header" style={{ marginBottom: 32 }}>
        ...
        <h1 className="page-title">
          {isSkipped ? '导入已有标注' : isLoadingMoondream ? 'Moondream 模型下载中' : '两段式打标进行中'}
        </h1>
      ...
      <div className="card-section" style={{ marginBottom: 24 }}>
        ...
        <div style={{ marginBottom: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
            <span style={{ fontSize: 13, color: 'var(--gray-600)' }}>
              {isSkipped ? '标注文件处理进度' : isLoadingMoondream ? '模型加载中' : '处理进度'}
            </span>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
          ...
          <span style={{ fontSize: 11, color: 'var(--gray-400)' }}>
            {isSkipped ? '直接导入中，请稍候' : isLoadingMoondream ? '下载完成后自动继续' : '自动处理中，请勿关闭页面'}
          </span>
        </div>
      </div>
